        ...
        for (float j = 0; j < size; j++) {
            for (float k = 0; k < size; k++) {
                if (perlin.noise3D(i*scale/size,j*scale/size,k*scale/size) > threshold){
                    ...
                    model->place_voxel_at_point(vec3{i,j,k},Voxel(
                        vec3(
                            220 + rand_adjust_red,
                            60 + rand_adjust_gb,
                            60 + rand_adjust_gb
...
VoxelSpace<Voxel>* Scene::magicaVoxel(std::string file_name) {
    ...
    delete[] buffer;
    std::vector<std::pair<vec3,Voxel>> voxels;
    ...
    for (int i = 0; i < scene->num_instances; i++) {
        ...
        mat4 transform = make_mat4(reinterpret_cast<float*>(&mat));
        ...
        for (float k = 0; k < model->size_z; k++) {
            ...
            if (color_index != 0) {
                ...
                c /= 255.0f;
                vec3 loc = vec3(transform*vec4{i,j,k,1.0});
                loc = vec3(loc.x, loc.z, loc.y);
    ...
    auto xExtremes = std::minmax_element(voxels.begin(), voxels.end(),
                                     [](const std::pair<vec3,Voxel>& lhs, const std::pair<vec3,Voxel>& rhs) {
