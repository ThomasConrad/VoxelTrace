...
class VoxelSpace {
    ...
    vec3 octree_index_to_point(uvec3 const& oct_index) {
        return vec3(oct_index)*octree_to_world + bounding_box.min;
    }
...
public:
    ...
    bool remove_voxel_at_point(vec3 const& point) {
        uvec3 oct_index;
        if (try_point_to_octree_index(point, oct_index)) {
    ...
    bool trace_aabb(Ray r, float &t,BBox aabb, bool getMax = false) {
        ...
        vec3 t0 = (aabb.min - r.origin) * invdir;
        vec3 t1 = (aabb.max - r.origin) * invdir;
    ...
    bool intersect(Ray const& ray, HitInfo &out) {        
        ...
        while (!voxel_hit && out.depth < octree.max_depth*octree.max_depth*octree.max_depth) {
            ...
            float t; 
            if (tx < ty) {
                t = tx; out_normal = vec3(-sign_dir_x, 0.0, 0.0);
            } else {
                t = ty; out_normal = vec3(0.0, -sign_dir_y, 0.0);
            }
            if (tz < t) {
                t = tz; out_normal = vec3(0.0, 0.0, -sign_dir_z);
            }
