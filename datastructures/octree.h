...
class OcTree {
	template<typename D> struct Node { //Doing a second template is unnecessary, but prevents complaining from VS about an ambiguous destructor
		...
		Node(Node<D>* parent, char subindex, uint depth) : parent{ parent }, subindex_in_parent{ subindex }, depth{ depth } {
			for (int i = 0; i < 8; i++) {
				children[i] = nullptr;
			}
			data = nullptr;
	...
	char make_subindex_from_index(uint x, uint y, uint z, uint depth) {
	...
	static char compose_subindex(uint x_bit, uint y_bit, uint z_bit) {
		uint out = 0;
		if (x_bit) out += 4;
		if (y_bit) out += 2;
		if (z_bit) out += 1;
...
public:
	...
	//Returns tuple with following info: (was there an item at the location, at what depth did the loop end, and the returned item)
	std::tuple<bool, uint, T> get(uint x, uint y, uint z) {
		Node<T>* current_node = &root;
		char subindex;
		for (uint d = 0; d < max_depth; d++) {
			subindex = make_subindex_from_index(x, y, z, d);
			if (!current_node->child_exists.test(subindex)) {
				return std::tuple<bool, uint, T>(false, d, T());
			}
			current_node = current_node->children[subindex];
		}
		subindex = make_subindex_from_index(x, y, z, max_depth);
		if (current_node->child_exists.test(subindex)) {
			return std::tuple<bool, uint, T>(true, max_depth, current_node->data[subindex]);
		}
		return std::tuple<bool, uint, T>(false, max_depth, T());
	}
};