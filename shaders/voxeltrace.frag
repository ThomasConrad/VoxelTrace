...
vec3 skybox(vec3 dir)
{
  ...
  vec3 p_sunset_dark[4] = vec3[4](vec3(0.3720705374951474, 0.3037080684557225, 0.26548632969565816),
  ...
  vec3 p_sunset_bright[4] = vec3[4](vec3(0.38976745480184677, 0.31560358280318124, 0.27932656874),
  ...
  vec3 p_day[4] = vec3[4](vec3(0.051010496458305694, 0.09758747153634058, 0.14233364823001612),
  ...
  /* Sky */
  {
    ...
    vec3 p_sunset[4] = vec3[4](mix(p_sunset_dark[0], p_sunset_bright[0], brightness_d),
...
vec3 intersect(vec3 orig, vec3 dir, BBox bbox)
{
  ...
  vec3  n      = vec3(0.);
  vec3  invdir = 1.0 / dir;
  if(isInside(orig, bbox) || traceAABB(orig, invdir, t, n, bbox))
  {  // Either inside or trace to the bounding box
    ...
    if(trace(pos, dir, invdir, hit))
    {
      ...
      vec3 nn = vec3(0.);
      // check border
      if(!isInside(hit.pos * size + nodeMin + n * 1e-4 * t, bbox) && dot(ubo.lightDirection, n) > 0.)
      {
