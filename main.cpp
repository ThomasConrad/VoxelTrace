...
private:
    ...
    static void keyboard_button_callback(GLFWwindow* window, int key, int scancode, int action, int mods){
        ...
        auto app = reinterpret_cast<HelloTriangleApplication*>(glfwGetWindowUserPointer(window));
        app->tracer->keypress(key,action);
    ...
    //Create image buffer from file
    void createTextureImage(float time = 1.0){
        std::cout << "width: " << tracer->width << " " << tracer->height << std::endl;
        createImage(tracer->width, tracer->height, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, textureImage, textureImageMemory);
    }
    ...
    void updateBuffers(uint32_t currentImage) {
        ...
        tracer->input_update(delta_time);
        lastTime = currentTime;
        tracer->render(pixels, delta_time);

        vkMapMemory(device, textureImageMemory, 0, imageSize, 0, &data);
            memcpy(data, pixels, static_cast<size_t>(imageSize));
        vkUnmapMemory(device, textureImageMemory);
        //Transitioning the image layout is slow. Maybe add it to the pipeline? Right now it's just for testing anyways.updateBu
    ...
    //Try to find best format, which is srgb, otherwise just the first format
