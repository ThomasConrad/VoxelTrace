...
#define GLM_FORCE_DEPTH_ZERO_TO_ONE //Opengl is -1,1, whereas vulkan is 0,1. 
...
private:
    ...
    void loadModel(){
        ...
        std::string path = prefix + '/' + MODEL_PATH;
        if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, path.c_str())) {
            throw std::runtime_error(warn + err);
        }

        std::unordered_map<Vertex, uint32_t> uniqueVertices{}; //map to hold unique vertices
        for (const auto& shape : shapes) {
             for (const auto& index : shape.mesh.indices) {
                ...
                vertex.texCoord = {
                    ...
                    1.0f - attrib.texcoords[2 * index.texcoord_index + 1] //Swap y axis because vulkan
                };
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        std::array<VkClearValue, 2> clearValues{}; //Clear color to black and depth stencil to 1=far plane, 0=near plane
    ...
    void createTextureImage(){
        ...
        std::string prefix = SRCDIR;
        std::string path = prefix + '/' + TEXTURE_PATH;
